...
in
{
  ...
  */
  options.fnltochkaLib.desktop.sunshine.allowWan = lib.mkEnableOption "WAN access to Sunshine";
  ...
  config = lib.mkIf (cfg.enable && cfg.sunshine.enable) {
    services.sunshine = {
      ...
      openFirewall = true;
      settings = {
        origin_web_ui_allowed = if cfg.sunshine.allowWan then "wan" else "lan";
      };
    };
  };
}
