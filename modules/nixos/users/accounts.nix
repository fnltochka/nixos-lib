  ...
  */
  ...
  config =
    let
      ...
      defaultShell = if zshEnabled then pkgs.zsh else pkgs.bash;
    ...
    in
    {
      users.users = lib.mapAttrs' (
        ...
        lib.nameValuePair username (
          lib.mkMerge [
            ...
            {
              shell = lib.mkForce (if isZsh u.shell && !zshEnabled then defaultShell else u.shell);
            }
          ]
        )
      ) enabledAccounts;
      ...
      home-manager.users = lib.mkIf hmEnabled (
        lib.mapAttrs' (username: u: lib.nameValuePair username u.homeModule) (
