...
in
{
  /**
    ...
    ```
  */
  options.fnltochkaLib.hardware.nvidia.enable =
    lib.mkEnableOption "NVIDIA GPU (CUDA, container toolkit, OBS/Sunshine overrides)";
  ...
  */
  options.fnltochkaLib.hardware.nvidia.reducedVaSpaceWorkarounds =
    lib.mkEnableOption "VA space workarounds (kernel params + Mutter software cursor) for affected GPUs only";
